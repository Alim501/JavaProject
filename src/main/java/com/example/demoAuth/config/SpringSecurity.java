...
public class SpringSecurity {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                .requestMatchers("/register/**").permitAll()  // Allow access to registration URLs
                .requestMatchers("/index").permitAll()      // Allow access to index page
                .requestMatchers("/error").permitAll()
                .requestMatchers("/**").authenticated() // Require ADMIN role for /users page
//                .requestMatchers("/category/**").authenticated() // Require ADMIN role for /users page
//                .requestMatchers("/tasks/**").authenticated() // Require ADMIN role for /users page
                .and()
                    .formLogin(form -> form
                            .loginPage("/login") // Custom login page
                            .loginProcessingUrl("/login") // URL for processing login
                            .defaultSuccessUrl("/task", true) // Redirect to /tasks after successful login
